...
export default function AdminPage() {
  ...
  const [memberMsgPreset, setMemberMsgPreset] = useState<"시험 독려" | "루틴 독려" | "합격 축하" | "">("");
  const [memberMsgTitle, setMemberMsgTitle] = useState("");
  const [memberMsgBody, setMemberMsgBody] = useState("");
  const [memberMsgTarget, setMemberMsgTarget] = useState<"selected" | "plan">("selected");
  ...
  const [memberMsgProbeResult, setMemberMsgProbeResult] = useState<PushDebugInfo | null>(null);
  ...
  const memberRecentMap = useMemo(() => {
    ...
    for (const item of [...attempts].sort((a, b) => {
      const ta = toDate(a.created_at)?.getTime() || 0;
      const tb = toDate(b.created_at)?.getTime() || 0;
      return tb - ta;
    })) {
  ...
  const stats = useMemo(() => {
    ...
    const todayQuiz = attempts.filter((item) => String(item.created_at || "").slice(0, 10) === todayKey).length;
    return {
      totalMembers,
      proMembers,
      adminMembers,
  ...
  const handleProbePushTarget = async () => {
    try {
      ...
      console.error(error);
      setPushMessage(error instanceof Error ? error.message : "대상 확인 중 오류가 발생했습니다.");
    } finally {
  ...
  const handleSendPush = async () => {
    ...
    try {
      ...
      console.error(error);
      setPushMessage(error instanceof Error ? error.message : "푸시 발송 중 오류가 발생했습니다.");
    } finally {
      setPushBusy(false);
    }
  };
  ...
  const handleCleanupPreview = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(String(data?.error || "정리 미리보기 중 오류가 발생했습니다."));
      ...
      console.error(error);
      setCleanupMessage(error instanceof Error ? error.message : "정리 미리보기 중 오류가 발생했습니다.");
    } finally {
  ...
  const handleCleanupRun = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(String(data?.error || "기록 정리 중 오류가 발생했습니다."));
      ...
      console.error(error);
      setCleanupMessage(error instanceof Error ? error.message : "기록 정리 중 오류가 발생했습니다.");
    } finally {
      setCleanupBusy(false);
    }
  };

  const handleApplyMemberMsgPreset = (preset: "시험 독려" | "루틴 독려" | "합격 축하") => {
    setMemberMsgPreset(preset);
    if (preset === "시험 독려") {
      setMemberMsgTitle("시험, 끝까지 잘 해봅시다");
      setMemberMsgBody("조금만 더 집중해 봅시다. 지금까지 쌓아온 힘이 분명히 결과로 이어질 거예요.");
    } else if (preset === "루틴 독려") {
      setMemberMsgTitle("오늘도 루틴, 가볍게 이어가요");
      setMemberMsgBody("길게 하지 않아도 괜찮습니다. 오늘도 잠깐이라도 이어가면 실력은 분명히 앞으로 갑니다.");
    } else {
      setMemberMsgTitle("정말 축하합니다");
      setMemberMsgBody("좋은 결과, 진심으로 축하드립니다. 여기까지 해낸 과정 자체가 정말 대단합니다.");
    }
    setMemberMsgStatus("");
  ...
  const handleProbeMemberMessageTarget = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(String(data?.error || "대상 수 확인 중 오류가 발생했습니다."));
      ...
      console.error(error);
      setMemberMsgStatus(error instanceof Error ? error.message : "대상 수 확인 중 오류가 발생했습니다.");
    } finally {
  ...
  const handleSendMemberMessage = async () => {
    ...
    try {
      ...
      console.error(error);
      setMemberMsgStatus(error instanceof Error ? error.message : "메시지 전송 중 오류가 발생했습니다.");
    } finally {
  ...
  const handleBackupZip = async () => {
    ...
    try {
      ...
      console.error(error);
      setBackupMessage(error instanceof Error ? error.message : "백업 ZIP 생성 중 오류가 발생했습니다.");
    } finally {
      setBackupBusy(false);
    }
  };
        ...
        {tab === "members" ? (
          <section className="mt-8">
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-sm text-gray-600">기존 플랜 변경 UI는 유지하고, 하단 기록 정리와 메시지 발송까지 연결했습니다.</p>
            </div>
            ...
            {memberMessage ? (
              ...
              <p className="mt-6 text-sm text-gray-600">검색 결과: {filteredProfiles.length}명 / 전체: {profiles.length}명</p>
            )}
            ...
            <div className="mt-10 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
              <div>
                ...
                <p className="mt-2 text-sm text-gray-600">선택 회원 또는 특정 플랜 전체에게 메시지를 보낼 수 있습니다.</p>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  {memberMsgTarget === "plan" ? (
                    <p className="mt-3 text-sm text-gray-600">현재 선택: {memberMsgPlan} 전체</p>
                  ) : null}
                </div>
              </div>
              ...
              {memberMsgStatus ? (
                ...
                <p className="text-sm text-gray-500">전송 전에 대상 수 미리보기를 눌러 확인하면 더 안전합니다.</p>
              )}
            </div>
          </section>
        ) : null}
        ...
        {tab === "stats" ? (
          <section className="mt-8 space-y-8">
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-sm text-gray-600">현재 기록을 기준으로 앱 사용 패턴을 간단히 봅니다.</p>
            </div>
        ...
        {tab === "push" ? (
          <section className="mt-8 space-y-8">
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-sm text-gray-600">테스트 발송 또는 전체 발송을 선택해 바로 알림을 보낼 수 있습니다.</p>
            </div>
            <div className="space-y-6 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              <div>
                ...
                <div className="mt-3 flex flex-wrap gap-3">
                  <button type="button" onClick={() => { setPushMode("test"); setPushMessage(""); setPushDebugInfo(null); }} className={pushMode === "test" ? "rounded-2xl bg-black px-4 py-3 text-sm font-semibold text-white" : "rounded-2xl border border-gray-300 bg-white px-4 py-3 text-sm font-semibold text-gray-800"}>테스트 발송</button>
                  <button type="button" onClick={() => { setPushMode("selected"); setPushMessage(""); setPushDebugInfo(null); }} className={pushMode === "selected" ? "rounded-2xl bg-black px-4 py-3 text-sm font-semibold text-white" : "rounded-2xl border border-gray-300 bg-white px-4 py-3 text-sm font-semibold text-gray-800"}>선택 회원 발송</button>
                  <button type="button" onClick={() => { setPushMode("all"); setPushMessage(""); setPushDebugInfo(null); }} className={pushMode === "all" ? "rounded-2xl bg-black px-4 py-3 text-sm font-semibold text-white" : "rounded-2xl border border-gray-300 bg-white px-4 py-3 text-sm font-semibold text-gray-800"}>전체 발송</button>
                </div>
              </div>
              ...
              {pushMode === "selected" ? (
                <div className="space-y-4 rounded-2xl border border-gray-200 bg-gray-50 p-4">
                  ...
                  <div>
                    ...
                    <select value={selectedPushUserId} onChange={(e) => { setSelectedPushUserId(e.target.value); setPushMessage(""); setPushDebugInfo(null); }} className="mt-3 w-full rounded-2xl border border-gray-200 bg-white px-4 py-4 text-base outline-none">
                      <option value="">회원 선택</option>
                      {filteredPushProfiles.map((item) => (
                        <option key={item.id} value={item.id}>{item.email} · {item.plan}</option>
                      ))}
                    </select>
                  </div>
                  {selectedPushUserId ? <p className="text-sm text-gray-600">선택됨: {selectedPushProfile?.email || "-"}</p> : null}
                </div>
              ) : null}

              <div>
                <label className="text-base font-semibold text-gray-800">제목</label>
                <input value={pushTitle} onChange={(e) => { setPushTitle(e.target.value); setPushMessage(""); setPushDebugInfo(null); }} className="mt-3 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-4 text-base outline-none" />
              </div>
              <div>
                <label className="text-base font-semibold text-gray-800">내용</label>
                <textarea value={pushBody} onChange={(e) => { setPushBody(e.target.value); setPushMessage(""); setPushDebugInfo(null); }} rows={4} className="mt-3 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-4 text-base outline-none" />
              </div>
              <div>
                <label className="text-base font-semibold text-gray-800">클릭 이동 URL (선택)</label>
                <input value={pushUrl} onChange={(e) => { setPushUrl(e.target.value); setPushMessage(""); setPushDebugInfo(null); }} className="mt-3 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-4 text-base outline-none" />
              </div>

              <div className="flex flex-wrap gap-3">
                <button type="button" onClick={handleProbePushTarget} disabled={pushProbeBusy || (pushMode === "selected" && !selectedPushUserId)} className={pushProbeBusy || (pushMode === "selected" && !selectedPushUserId) ? "inline-flex rounded-2xl border border-gray-200 bg-gray-100 px-5 py-4 text-base font-semibold text-gray-400" : "inline-flex rounded-2xl border border-gray-300 bg-white px-5 py-4 text-base font-semibold text-gray-800"}>{pushProbeBusy ? "대상 확인 중..." : "대상 확인"}</button>
                <button type="button" onClick={handleSendPush} disabled={pushBusy || !pushTitle.trim() || !pushBody.trim() || (pushMode === "selected" && !selectedPushUserId)} className={pushBusy || !pushTitle.trim() || !pushBody.trim() || (pushMode === "selected" && !selectedPushUserId) ? "inline-flex rounded-2xl border border-gray-200 bg-gray-100 px-6 py-4 text-base font-semibold text-gray-400" : "inline-flex rounded-2xl bg-black px-6 py-4 text-base font-semibold text-white"}>{pushBusy ? "푸시 발송 중..." : pushMode === "test" ? "🚀 테스트 발송" : pushMode === "selected" ? "🚀 선택 회원 발송" : "🚀 전체 발송"}</button>
              </div>

              {pushMessage ? <p className="text-sm text-gray-600">{pushMessage}</p> : <p className="text-sm text-gray-500">테스트 발송은 현재 관리자 계정의 구독만, 선택 회원 발송은 지정 회원의 구독만, 전체 발송은 저장된 전체 구독을 대상으로 합니다.</p>}
        ...
        {tab === "logs" ? (
          <section className="mt-8 space-y-8">
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-sm text-gray-600">최근 시도 기록을 카드형으로 확인하고 필터로 빠르게 찾을 수 있습니다.</p>
            </div>
            ...
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
              ...
              <p className="text-sm text-gray-600">필터 결과: {filteredLogs.length}건 / 전체 최근 기록: {stats.recentLogs.length}건</p>
            </div>

            <div className="space-y-4">
              {filteredLogs.map((item, idx) => (
                <div key={`${item.user_id || "log"}-${idx}-${item.created_at || ""}`} className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <span className="rounded-full border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-semibold text-gray-700">{item.level || "-"}</span>
                    <span className="rounded-full border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-semibold text-gray-700">{item.pos_mode || "-"}</span>
                    <span className="rounded-full border border-green-200 bg-green-50 px-3 py-2 text-sm font-semibold text-gray-700">✅ {item.score}/{item.quiz_len}</span>
                    <span className="rounded-full border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-gray-700">❌ {item.wrong_count || 0}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab === "backup" ? (
          <section className="mt-8 space-y-8">
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-sm text-gray-600">현재 핵심 파일을 ZIP으로 백업하는 흐름을 정리하는 자리입니다.</p>
            </div>
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              <label className="text-base font-semibold text-gray-800">버전 태그</label>
              <input value={backupTag} onChange={(e) => { setBackupTag(e.target.value); setBackupMessage(""); }} className="mt-3 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-4 text-base outline-none" />
              <label className="mt-5 flex items-center gap-3 text-sm font-medium text-gray-700">
                <input type="checkbox" checked={backupConfirm} onChange={(e) => { setBackupConfirm(e.target.checked); setBackupMessage(""); }} className="h-4 w-4" />
                백업 실행 확인
              </label>
              <button type="button" onClick={handleBackupZip} disabled={!backupTag.trim() || !backupConfirm || backupBusy} className={!backupTag.trim() || !backupConfirm || backupBusy ? "mt-6 inline-flex rounded-2xl border border-gray-200 bg-gray-100 px-6 py-4 text-base font-semibold text-gray-400" : "mt-6 inline-flex rounded-2xl bg-red-500 px-6 py-4 text-base font-semibold text-white"}>{backupBusy ? "백업 ZIP 생성 중..." : "백업 ZIP 만들기"}</button>
              {backupMessage ? <p className="mt-4 text-sm text-gray-600">{backupMessage}</p> : <p className="mt-4 text-sm text-gray-500">태그 입력 + 실행 확인 체크 후 버튼이 활성화됩니다.</p>}
            </div>
          </section>
        ) : null}
      </div>
