...
function normalizeKnownWordsToReading(text: string) {
  return String(text || "")
    ...
    .replace(/気をつけました/g, "きをつけました")
    .replace(/気をつけます/g, "きをつけます")
    .replace(/気をつけて/g, "きをつけて")
    .replace(/気をつける/g, "きをつける")
    .replace(/気をつけ/g, "きをつけ")
    .replace(/本当に/g, "ほんとに")
    ...
    // 장소/도시
    .replace(/駅/g, "えき")
    .replace(/美術館/g, "びじゅつかん")
    .replace(/町/g, "まち")
    .replace(/街/g, "まち")
    .replace(/名前/g, "なまえ")
    .replace(/人気/g, "にんき")
    .replace(/場所/g, "ばしょ")
    .replace(/横ですね/g, "よこですね")
    .replace(/横です/g, "よこです")
    .replace(/横/g, "よこ")
    ...
    .replace(/高い/g, "たかい")
    .replace(/高く/g, "たかく")
    .replace(/迷ったら/g, "まよったら")
    .replace(/迷います/g, "まよいます")
    .replace(/迷って/g, "まよって")
    .replace(/迷った/g, "まよった")
    .replace(/迷う/g, "まよう")
    .replace(/行かない/g, "いかない")
    .replace(/行かなくて/g, "いかなくて")
    .replace(/行かず/g, "いかず")
    .replace(/行けない/g, "いけない")
    ...
    .replace(/12時/g, "じゅうにじ")
    .replace(/遅れない/g, "おくれない")
    .replace(/遅れる/g, "おくれる")
    .replace(/遅れて/g, "おくれて")
    .replace(/遅れ/g, "おくれ")
    .replace(/心配です/g, "しんぱいです")
    .replace(/心配/g, "しんぱい")
...
export async function POST(req: Request) {
  try {
    ...
    const plainPrompt = [
      ...
      "이 전사는 일반 문장 표기보다 일본어 학습용 읽기 전사를 우선합니다.",
      "가능하면 문장 전체를 히라가나로 전사하세요.",
      "한자는 가능한 한 사용하지 마세요.",
      "의미를 바꾸지 말고, 들린 문장을 읽는 형태의 히라가나로 적으세요.",
      "예: 美味しい → おいしい, 思っていた → おもっていた, 名前 → なまえ, 私は → わたしは, 週末 → しゅうまつ",
      "실제로 들린 음성만 전사하세요.",
      "기침, 헛기침, 목을 가다듬는 소리, 한숨, 짧은 감탄 소리는 실제 들린 짧은 소리 그대로 적으세요.",
      "예: えへん、んー、うーん、えー、あー",
    ...
    const guidedPrompt = [
      ...
      "한자는 가능한 한 사용하지 마세요.",
      "의미를 바꾸지 말고, 들린 문장을 읽는 형태의 히라가나로 적으세요.",
      "예: 美味しい → おいしい, 思っていた → おもっていた, 名前 → なまえ, 私は → わたしは, 顔に出る → かおにでる, 週末 → しゅうまつ, 贅沢 → ぜいたく",
      "정답을 맞추려고 추측하지 말고, 실제로 들린 음성만 전사하세요.",
      "특히 조사(は/が/を/に/へ/で/と/も)는 절대로 문법적으로 보정하지 말고, 들린 그대로 적으세요.",
      "같은 뜻의 일반적인 표기 차이가 있을 때는 한자보다 히라가나 표기를 우선하세요.",
      "들리지 않거나 확실하지 않으면 추측하지 말고 짧게 전사하세요.",
    ]
      .filter(Boolean)
      .join("\n");
