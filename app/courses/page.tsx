...
type CatalogCard = {
  ...
  description: string;
  progress: number;
  catalogStatus: "enrolled" | "available" | "coming" | "completed";
  badgeLabel: string;
  buttonLabel: string;
  href: string;
...
function getBadgeClass(status: CatalogCard["catalogStatus"]) {
  if (status === "enrolled") return "bg-black text-white";
  if (status === "available") return "bg-gray-200 text-gray-800";
  if (status === "completed") return "bg-green-100 text-green-700";
  return "bg-white text-gray-500 ring-1 ring-gray-200";
}

function getButtonClass(status: CatalogCard["catalogStatus"]) {
  if (status === "enrolled") return "bg-black text-white hover:opacity-90";
  if (status === "available") {
...
function getCardClass(status: CatalogCard["catalogStatus"]) {
  if (status === "enrolled") {
...
export default function CoursesPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        console.error(error);
        if (mounted) {
          setCoursesError("강의 카탈로그를 불러오지 못했습니다.");
        }
      } finally {
        if (mounted) {
          setLoading(false);
  ...
  const courses: CatalogCard[] = useMemo(() => {
    const mapped = courseRows.map((course) => {
      ...
      if (enrollment?.is_completed || enrollment?.progress === 100) {
        return {
          ...
          description: course.description,
          progress: 100,
          catalogStatus: "completed" as const,
          badgeLabel: "수강 완료",
          buttonLabel: "복습하러 가기",
      ...
      if (enrollment && enrollment.progress > 0) {
        return {
          ...
          catalogStatus: "enrolled" as const,
          badgeLabel: "수강 중",
          buttonLabel: "이어서 학습",
    ...
    return mapped.sort((a, b) => priority[a.catalogStatus] - priority[b.catalogStatus]);
  }, [courseRows, enrollmentMap]);

  const continueCourse =
    courses.find((course) => course.catalogStatus === "enrolled") ?? null;
  const availableCount = courses.filter(
    (course) => course.catalogStatus === "available"
  ).length;
  const enrolledCount = courses.filter(
    (course) => course.catalogStatus === "enrolled"
  ).length;
  const completedCount = courses.filter(
    (course) => course.catalogStatus === "completed"
  ).length;
      ...
      <div className="mx-auto w-full max-w-6xl">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h1 className="mt-3 text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl">
                {loading
                  ? "강의 목록을 불러오는 중입니다."
                  : `${displayName}님을 위한 전체 강의 목록입니다.`}
              </h1>

              <p className="mt-3 max-w-2xl text-sm leading-6 text-gray-600 sm:text-base">
                어떤 강의가 열려 있는지 한눈에 보고, 수강 중인 강의는 바로 이어서 학습하고,
                아직 시작하지 않은 강의는 이곳에서 먼저 살펴볼 수 있습니다.
              </p>

              <div className="mt-4 flex flex-wrap gap-3">
                <span className="rounded-full border border-gray-200 bg-gray-50 px-4 py-2 text-sm font-semibold text-gray-700">
                  전체 강의 {courseRows.length}개
                </span>
                <span className="rounded-full border border-gray-200 bg-gray-50 px-4 py-2 text-sm font-semibold text-gray-700">
                  수강 가능 {availableCount}개
                </span>
                <span className="rounded-full border border-gray-200 bg-gray-50 px-4 py-2 text-sm font-semibold text-gray-700">
                  수강 중 {enrolledCount}개
                </span>
                <span className="rounded-full border border-gray-200 bg-gray-50 px-4 py-2 text-sm font-semibold text-gray-700">
                  완료 {completedCount}개
                </span>
            ...
            <div className="grid min-w-[260px] gap-3 sm:grid-cols-2 lg:w-[340px] lg:grid-cols-1">
              ...
              >
                {continueCourse ? "▶ 이어서 학습" : "내 강의실로 이동"}
              </Link>
              <Link
                href="/classroom"
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-3">
          <div className="rounded-[24px] border border-gray-200 bg-white p-5 shadow-sm">
            <p className="text-sm font-semibold text-gray-500">지금 이어볼 강의</p>
            <p className="mt-2 text-lg font-bold text-gray-900">
              {continueCourse ? continueCourse.title : "아직 이어볼 강의가 없습니다."}
            </p>
            <p className="mt-2 text-sm leading-6 text-gray-600">
              {continueCourse
                ? `${continueCourse.progress}% 진행 중 · ${
                    continueCourse.lastLessonTitle ?? "최근 기록"
                  }`
                : "강의를 시작하면 여기에서 바로 이어서 들어갈 수 있습니다."}
            </p>
          </div>
          ...
          <div className="rounded-[24px] border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-lg font-bold text-gray-900">
              {continueCourse?.lastStudiedAt
                ? formatDate(continueCourse.lastStudiedAt)
                : "기록 없음"}
            </p>
            <p className="mt-2 text-sm leading-6 text-gray-600">
              수강 중인 강의가 있다면 마지막 학습 시점이 이곳에 표시됩니다.
            </p>
          </div>

          <div className="rounded-[24px] border border-gray-200 bg-white p-5 shadow-sm">
            <p className="text-sm font-semibold text-gray-500">카탈로그 상태</p>
            <p className="mt-2 text-lg font-bold text-gray-900">
              {coursesLoading ? "불러오는 중" : coursesError ? "점검 필요" : "정상 표시 중"}
            </p>
            <p className="mt-2 text-sm leading-6 text-gray-600">
              {coursesError || "공개된 강의만 자동으로 노출되도록 연결되어 있습니다."}
            </p>
          </div>
        </section>
        ...
        <section className="mt-6 rounded-[28px] border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-gray-500">전체 강의</p>
              <h2 className="mt-2 text-2xl font-bold text-gray-900">
                수강생용 강의 카탈로그
              </h2>
            </div>
            <p className="text-sm text-gray-500">
              수강 상태에 따라 카드 버튼이 달라집니다.
            </p>
          </div>
          ...
          {coursesLoading ? (
            ...
            <div className="mt-6 rounded-2xl border border-gray-200 bg-gray-50 p-6 text-sm text-gray-600">
              아직 표시할 강의가 없습니다. 공개 강의를 추가해 주세요.
            </div>
          ) : (
            <div className="mt-6 grid gap-4 xl:grid-cols-3">
              {courses.map((course) => (
                <article key={course.id} className={getCardClass(course.catalogStatus)}>
                  <CourseThumbnail
                    src={course.thumbnailUrl}
                    title={course.title}
                    emphasize={course.catalogStatus === "enrolled"}
                  />
                  ...
                  <div className="mt-4 rounded-2xl bg-white p-4 ring-1 ring-gray-200">
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      {course.lastStudiedAt ? formatDate(course.lastStudiedAt) : "기록 없음"}
                    </p>
                  </div>
